...
# i18n - extract UI strings AND sync desktop-entry strings into po/
# (runs xgettext + tools/sync_desktop_po.py, which also regenerates po/LINGUAS)
make update-po
...
Desktop-entry strings (`GenericName`, `Comment` in `data/dedisc.desktop.in`)
are NOT extracted by xgettext. They live as plain (context-less) msgids in
every `po/*.po` and are merged at install time by `msgfmt --desktop`
(see the Makefile). You do NOT maintain them by hand: `tools/sync_desktop_po.py`
appends missing entries to all catalogs automatically - it runs as part of
`make install` and `make update-po` (use its `--check` flag in CI). When
adding a translatable key to the desktop file, add it to `TRANSLATABLE_KEYS`
in the script AND to the `-k` allow-list in the Makefile's msgfmt call.
`po/LINGUAS` is regenerated by the script; do not edit it manually.
